...
public class RepairService {
    ...
    public Result<List<RepairOrder>> getAllOrders() {
        try {
            ...
            System.out.println("=== 查询到报修单数量：" + list.size() + " ===");

            for (RepairOrder order : list) {
                User user = userMapper.selectById(order.getStudentId());
                System.out.println("学生ID: " + order.getStudentId() + ", 用户: " + user);

                if (user != null) {
                    order.setStudentAccount(user.getAccount());
                    System.out.println("设置账号：" + user.getAccount());  // ← 看这行有没有输出
                }
            }
    ...
    public Result<List<RepairOrder>> getOrdersByStatus(Integer status) {
        try {
            LambdaQueryWrapper<RepairOrder> wrapper = new LambdaQueryWrapper<>();
